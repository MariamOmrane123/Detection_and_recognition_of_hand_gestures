...
while(cap.isOpened()): #while the default camera is opened(m)
    ...
    if count_defects == 1:#two fingers 
        if(holdDown==0):#if unmute
            ...
            holDown==0
            cv2.putText(img,"unmute", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
            time.sleep(1)
    elif count_defects == 2:#three fingers
        pyautogui.press('volumeup')#volume up
        cv2.putText(img,"volume up", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
    elif count_defects == 3:#four fingers
        pyautogui.press('volumedown')#volume down
        cv2.putText(img,"volume down", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
    elif count_defects == 4:#five fingers
        pyautogui.press('space')#play or pause
        time.sleep(1)
    else:#nothing
        pyautogui.press('nothing')
